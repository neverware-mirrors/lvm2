 ...
 */
struct settings {
	uint64_t metadata_offset;  /* start of text metadata, from start of disk, in bytes */
	uint64_t mda_offset;       /* start of mda_header, from start of disk, in bytes */
	uint64_t mda_size;         /* size of metadata area (mda_header + text area), in bytes */
	uint64_t mda2_offset;
	uint64_t mda2_size;
	uint64_t device_size;      /* in bytes */
	uint64_t data_offset;      /* start of data (pe_start), in bytes */
	uint32_t seqno;
	struct id pvid;
			   ...
			   uint64_t *mda2_offset, uint64_t *mda2_size)
{
	...
	char buf2[256];
	char *buf;
	uint64_t mda_offset, mda_size, extra_bytes;
	int i, found = 0;
